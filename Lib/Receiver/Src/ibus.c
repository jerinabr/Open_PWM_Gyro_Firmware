...
/*!
  ...
  @details  Every IBus frame has a packet length byte, command byte, some amount of
  payload bytes, and 2 bytes for a 16-bit checksum, in that order

  The first state checks to see if the first byte (length byte) is at least 4 bytes,
  since the frame has 4 bytes + n payload bytes, and at most 32 bytes, since the IBus
  protocol CURRENTLY sends 14 channels over (2 bytes per channel)

  If this is valid, then the state machine proceeds with reading the command byte,
  the payloads bytes, and the 2 checksum bytes

  If the command code is a channel data command (0x40), then convert the payload data
  into channel data and indicate the received data was valid
*/
      ...
      if (checksum_calc == checksum_rx) {
        if (cmd == SERVO_CMD_CODE) {
          for (int i = 0; i < MAX_CHANNELS; i++) {
            rx.channel_data[i] = (ibus_buf[2*i + 1] << 8) | ibus_buf[2*i];
          }
