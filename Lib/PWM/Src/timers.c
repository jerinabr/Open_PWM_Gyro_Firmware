...
/*!
    ...
    @details All 4 channels of timer 2 are used but only channels 1 and 2 of
    timer 4 are used so the other 2 channels of timer 4 aren't configured

    The timers start with the pin outputs held low until the timer compare
    registers contain values greater than 0
*/
static void configure_timers(void) {
    /*
        Set prescaler to 159 for a 1MHz counter clock

        Section 30.5.14 of the reference manual states that the counter clock
        frequency is equal to f_tim_psc_ck / (PSC[15:0] + 1), which in our case
        would be 160MHz / (159 + 1) = 1MHz

        This makes the counter increment every 1us which is very convenient
        because that means the PWM pulse width (in microseconds) is the value in
    ...
    */
    ...
    /*
        ...
        50Hz has a period of 20000us and the counter updates every 1us. However,
        since the counter starts at 0, the counter needs to overflow at 19999
        for a total of 20000 counts
    */
    ...
    /*
        ...
        The ensures that any updates to the PWM pulse widths only take effect
        AFTER a cycle has completed and not DURING the cycle
    */
...
/*!
    ...
    The compare registers are also preloaded which means that the new values
